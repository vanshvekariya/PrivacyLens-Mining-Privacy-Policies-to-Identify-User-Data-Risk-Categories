...
def heading(text):
    para(text, bold=True, space_before=9, space_after=3)
...
para("Privacy policies are the main way companies disclose how they "
     "collect, use, share, and retain user data, but they are usually too "
     "long and complex for ordinary users to read. Only a small share of "
     "adults consistently read privacy policies before accepting them [2], "
     "and policies grew steadily longer and harder to read between 1996 "
     "and 2021 [3]. Users therefore agree to terms without noticing "
     "important clauses about data sharing, advertising, retention, or "
     "deletion rights.")
para("**PrivacyLens** treats this as a supervised text-classification "
     "task: given a privacy-policy segment, the system predicts which "
     "privacy-practice categories it expresses and presents the result in "
     "a form meant for ordinary readers (simplified category, risk level, "
     "reading priority, and highlighted supporting text). The goal is not "
     "legal advice but triage, helping users decide which parts of a "
     "policy deserve attention.")
para("Our dataset is the **OPP-115 corpus** [1]: 115 website privacy "
     "policies annotated with ten data-practice categories by law "
     "students. Its consolidated annotations yield 3,791 segments, and we "
     "map the ten legal categories to seven user-facing ones (Table 1).")
para("The overall problem is the same as in our proposal, but working with "
     "the dataset led us to refine it in three ways. First, "
     "the task is now explicitly **multi-label**: 49.1% of segments carry "
     "more than one category (1.64 labels per segment on average), a risk "
     "the proposal had flagged, so we make one binary decision per "
     "category instead of forcing a single label. The characteristics of "
     "the dataset make this a necessity rather than simply a modeling "
     "choice. Second, advertising/profiling "
     "is not a top-level OPP-115 practice; it appears as a purpose "
     "attribute within the collection and sharing practices, so such "
     "clauses fall under Data Collection or Third-Party Sharing, and a "
     "separate advertising category is left as future work. Third, because "
     "OPP-115 predates GDPR and CCPA, we added a small **generalization "
     "set**: 100–200 hand-labeled segments from 5–10 modern (2025–2026) "
     "policies, used purely as an additional evaluation set.")
...
bullet("**RQ2:** How much does a fine-tuned transformer improve over "
       "rule-based and classical bag-of-words models on this task, and "
       "why?")
bullet("**RQ3:** How well do OPP-115-trained models generalize to modern "
       "services' policies, and which categories transfer poorly?",
       last=True)
para("Success is measured quantitatively. Our headline metric is "
     ...
     "categories. Because missing a third-party-sharing clause is worse "
     "than misclassifying a policy-change sentence, we track recall on "
     "high-risk categories separately. Our "
     ...
     "out-of-fold predictions. As a result, the per-category F1 values in "
     "Table 3 do not average exactly to the fold-averaged macro-F1 in "
     "Table 2.")
para("The evaluation is designed so that each possible outcome helps "
     "answer one of the research questions. If the transformer performs "
     "much better than the linear models, it suggests contextual "
     "understanding is important for interpreting privacy language. "
     "Otherwise, it would indicate that simpler lexical features already "
     "capture most of the information needed. Either outcome provides "
     "evidence for answering RQ2. The generalization experiment will "
     "likewise either support using OPP-115-trained models on current "
     "policies or quantify how much post-GDPR language has drifted.")
...
bullet("Two goals aimed at end users extend the original interpretability "
       "goal: a literature-backed risk mapping grounded in privacy "
       "nutrition-label research [6], ToS;DR criteria, and GDPR "
       "principles; and a reading-priority recommendation (1–5 stars) "
       "combining a category's risk tier with prediction confidence, "
       "turning a long policy into a short, prioritized reading list.")
...
para("**Sources consulted.** As promised in the proposal, we consulted the "
     "literature before running experiments: the OPP-115 paper [1], early "
     "privacy-policy classification work [7], Polisis [4], PrivBERT [5], "
     "and privacy nutrition labels [6]. These shaped the multi-label "
     "framing, the choice of a transformer as the advanced model, the "
     "grounding of the risk mapping, and our positioning as a user-facing "
     "layer (risk, reading priority, evidence highlighting) rather than "
     "taxonomy labels alone.")
...
bullet("Structured error analysis organized into recurring failure types, "
       "plus evidence highlighting: top-weighted TF-IDF n-grams for the "
       "linear models and a simple word-occlusion score for the "
       "transformer.")
...
para("**Updated timeline.** July 10–14: finalize the classical analysis "
     "and begin transformer fine-tuning. July 14–19: transformer "
     "experiments and structured error analysis. July 19–26: evidence "
     "highlighting, risk/priority layer, modern-policy labeling, and the "
     "generalization evaluation. July 26–31: prototype, final report, and "
     "presentation; the optional LLM baseline only if time remains.")
...
bullet("Dataset understanding, label mapping, text preprocessing, the "
       "policy-disjoint split, and Linear SVM experiments (done).")
...
bullet("Majority and keyword baselines, Naive Bayes experiments, and the "
       "shared evaluation harness (metrics, cross-validation, confusion "
...
bullet("TF-IDF feature extraction and Logistic Regression experiments "
       "(done).")
...
bullet("Visualization and report assembly; her share of the generalization "
       "labeling.", last=True)
para("**Changes since the proposal:** Mansoor Ali owned the tree-model "
     "experiments, model evaluation, and error analysis. The tree models "
     "were removed (Section 3); the evaluation harness moved to Lakshita "
     "and error-analysis coordination to Aishwarya.")

# ---------------- 5. Initial results ----------------
heading("5. Initial results")
para("**Dataset.** Table 1 summarizes the corpus after parsing and mapping: "
     "3,791 annotated segments across 115 policies (33 per policy on "
     "average, mean length 70 words). Two findings directly affected the "
...
para("**Model comparison.** Table 2 reports 5-fold cross-validation, "
     "grouped by policy, on the training split; the held-out test set has "
...
bullet("Both linear models beat the keyword baseline by about 0.18 "
       "macro-F1, which suggests they capture patterns beyond the obvious "
       "vocabulary and gives a first partial answer to RQ2.")
bullet("**Naive Bayes underperformed the keyword baseline on macro-F1** "
       "despite a higher micro-F1. This is expected and does not indicate "
       "an implementation error: Multinomial Naive Bayes does not support "
       ...
       "class-weighted linear models shows what imbalance handling buys.",
       last=True)
...
caption("Table 3: Linear SVM per-category precision/recall/F1 on pooled "
        "out-of-fold predictions. Supports count the training split only "
        "(3,053 segments), hence lower than Table 1's full-corpus counts.")

fig_p = doc.add_paragraph()
fig_p.alignment = WD_ALIGN_PARAGRAPH.CENTER
fig_p.add_run().add_picture(str(FIGS / "fig_label_coprediction.png"),
                            width=Inches(3.1))
caption("Figure 1: Gold-label-conditioned prediction rates (Linear SVM, "
        "out-of-fold). For segments whose gold set is exactly one category, "
        "each cell is the share that also received each predicted label. "
        "Because the task is multi-label, this is not a conventional, "
        "mutually exclusive confusion matrix; per-category 2x2 matrices "
        "(TP/FP/FN/TN) are reported in results/per_category_confusion.csv. "
        "Off-diagonal mass shows label co-prediction; note the small Data "
        "Retention sample (n=6).")
...
para("**Error examples.** Two recurring failure types illustrate why "
     "privacy language is hard. (i) Cross-references: "
     ...
     "artifacts of simplifying ten legal categories into seven: a "
     "prediction can be semantically defensible yet count as wrong against "
     "the mapped gold label. We note this as a mapping limitation without "
     "using it to excuse genuine misclassifications.")
para("For completeness: under the strict exact-match criterion, 1,606 of "
     ...
     "the whole segment wrong, so values at this level are normal in "
...
heading("References")
refs = [
    "[1] S. Wilson et al. The Creation and Analysis of a Website Privacy "
    "Policy Corpus. In Proceedings of ACL, 2016.",
    ...
    "[4] H. Harkous et al. Polisis: Automated Analysis and Presentation of "
    "Privacy Policies Using Deep Learning. In Proceedings of USENIX "
    "Security, 2018.",
    ...
    "[6] P. G. Kelley et al. A “Nutrition Label” for Privacy. In "
    "Proceedings of SOUPS, 2009.",
    "[7] F. Liu et al. Towards Automatic Classification of Privacy Policy "
    "Text. Technical report, Carnegie Mellon University, 2018.",
]
for r in refs:
    para(r, size=9.5, space_after=1)
